const flashcards = {
    animals: [
        {
            image: "https://cdn-icons-png.flaticon.com/128/12734/12734110.png",
            word: "Wallet",
...
function nextFlashcard() {
    ...
    if (currentFlashcardIndex < flashcards[currentCategory].length) {
        if (!learnedWords.includes(flashcards[currentCategory][currentFlashcardIndex].word)) {
...
function updateFlashcard() {
    ...
    currentIndexDisplay.textContent = `${currentFlashcardIndex + 1} / ${flashcards[currentCategory].length}`; // Update the index display
...
function updateRemainingFlashcards() {
    const remainingCount = flashcards[currentCategory].length - currentFlashcardIndex - 1;
    document.getElementById("remainingFlashcards").textContent = `Remaining Flashcards: ${remainingCount}`;
}
...
function updateButtonStates() {
    ...
    const nextButton = document.getElementById("nextFlashcardButton");
    previousButton.disabled = currentFlashcardIndex === 0; // Disable if on the first card
    nextButton.disabled = currentFlashcardIndex >= flashcards[currentCategory].length - 1; // Disable if at the last card
}
